...
PARAM_ADDONS_PATH = "odoo_sh_community.addons_path"
PARAM_MY_MODULES_PATH = "odoo_sh_community.uploaded_modules_path"
...
class Module(models.Model):
    ...
    @api.model
    def _get_modules_with_changed_checksum_for_repo(self, repo=False):
        if not repo:
            return self._get_modules_with_changed_checksum()
        saved_checksums = self._get_saved_checksums()
        _logger.error('Modulos del repo---------->>>>>>' + str(repo.module_ids.mapped('name')))
        installed_modules = self.search([("state", "=", "installed"), ('name', 'in', repo.module_ids.mapped('name'))])
        _logger.error('Actualizar solo los modulos del repo---------->>>>>>' + str(installed_modules.mapped('name')))
        return installed_modules.filtered(
            lambda r: r._get_checksum_dir() != saved_checksums.get(r.name),
        )

    @api.model
    def upgrade_changed_checksum(self, repo=False, overwrite_existing_translations=True):
        ...
        output_list.append("Updating modules list...")
        self.update_list()
        # changed_modules = self._get_modules_with_changed_checksum()
        changed_modules = self._get_modules_with_changed_checksum_for_repo(repo)
        if not changed_modules and not self._get_modules_partially_installed():
            ...
            output_list.append("No checksum change detected in installed modules "
                               "and all modules installed, nothing to do.")
            return
        _logger.info(
        ...
        output_list.append("Marking the following modules to upgrade,for their checksums changed" + ",".join(
            changed_modules.mapped("name")))
        ...
        output_list.append("Upgrading...")
        self.env["base.module.upgrade"].upgrade_module()
        self.env.cr.commit()  # pylint: disable=invalid-commit

        _logger.info("Upgrade successful, updating checksums...")
        output_list.append("Upgrade successful, updating checksums...")
        self._save_installed_checksums()
        # pylint: disable=invalid-commit

        partial_modules = self._get_modules_partially_installed()
        if partial_modules:
            output_list.append("Checksum upgrade successful but incomplete for the following modules:" + ",".join(
                partial_modules.mapped("name")))
            self.env.cr.commit()
        ...
        output_list.append("Checksum upgrade complete.")
        if repo:
